...
import com.saf.core.services.ReviewService;
...
public class ReviewController {
    ...
    public ResponseEntity<ApiResponse<?>> createReview(@RequestBody ReviewRequest request) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.success("Đánh giá phòng thành công", review));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<?>> updateReview(@PathVariable Long id,
        ...
        try {
            var user = authorizationUtil.getCurrentUser();
            ReviewResponse review = reviewService.updateReview(user, id, request);
