...
import { User, Phone, Mail, Lock, Camera, Landmark, CreditCard, Copy, Check } from 'lucide-react';
...
const Profile = () => {
  ...
  const handleUpdateBank = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBankMsg(err?.response?.data?.message || 'Có lỗi xảy ra');
    } finally {
  ...
  const selectedBank = BANK_LIST.find(b => b.code === bankName);
  const vietQrUrl = selectedBank && bankAccountNumber && bankAccountHolder
    ? `https://img.vietqr.io/image/${bankName}-${bankAccountNumber}-compact2.jpg?accountName=${encodeURIComponent(bankAccountHolder)}`
    : null;
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      {/* Bank info — only for landlord */}
      {isLandlord && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-1">
            <Landmark className="w-4 h-4 text-blue-500" />
            <h2 className="font-bold text-gray-900 dark:text-white">Tài khoản ngân hàng</h2>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">
            Sinh viên sẽ thấy thông tin này sau khi xem phòng để chuyển khoản đặt cọc.
          </p>

          {bankMsg && (
            <div className={`mb-3 text-xs px-3 py-2 rounded-xl ${bankMsg.startsWith('✓') ? 'bg-green-50 dark:bg-green-900/20 text-green-600 border border-green-100 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 text-red-600 border border-red-100 dark:border-red-800'}`}>
              {bankMsg}
            </div>
          )}
          ...
          <form onSubmit={handleUpdateBank} className="space-y-3.5">
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Ngân hàng</label>
              <select
                value={bankName}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Số tài khoản</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tên chủ tài khoản</label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
              ...
              <p className="text-[11px] text-gray-400 mt-1">Nhập IN HOA, đúng tên trên tài khoản ngân hàng</p>
            </div>

            {/* Preview QR */}
            {vietQrUrl && (
              <div className="flex items-start gap-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                <img src={vietQrUrl} alt="VietQR" className="w-24 h-24 rounded-lg flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-gray-700 dark:text-gray-200">{selectedBank?.name}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <p className="text-sm font-bold text-gray-900 dark:text-white">{bankAccountNumber}</p>
                    <button type="button" onClick={handleCopyAccount} className="text-blue-500 hover:text-blue-700 transition-colors">
                      {copied ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{bankAccountHolder}</p>
                  <p className="text-[10px] text-gray-400 mt-1.5">QR này sinh viên có thể quét để chuyển tiền</p>
                </div>
              </div>
            )}
            ...
            <button type="submit" disabled={bankLoading}
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl disabled:opacity-60 transition-colors">
              {bankLoading ? 'Đang lưu...' : 'Lưu tài khoản ngân hàng'}
            </button>
          </form>
        </div>
      )}
